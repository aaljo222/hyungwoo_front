...
export class MouseCursor {
  ...
  _initStyle() {
    ...
    if (this.hasGsap) {
      ...
      if (this.center) {
        ...
        if (this.trailEl)
          gsap.set(this.trailEl, { force3D: true, opacity: 0.9 });
      }

      this.qx = gsap.quickTo(this.cursorEl, "x", {
        duration: this.speed,
        ease: this.ease,
      });
      this.qy = gsap.quickTo(this.cursorEl, "y", {
        duration: this.speed,
        ease: this.ease,
      });
      if (this.trailEl) {
        this.tx = gsap.quickTo(this.trailEl, "x", {
          duration: this.speed * 1.6,
          ease: this.ease,
        });
        this.ty = gsap.quickTo(this.trailEl, "y", {
          duration: this.speed * 1.6,
          ease: this.ease,
        });
      }
  ...
  _tick = () => {
    ...
    if (this.trailEl) {
      ...
      const ty = this.center
        ? this.cy - this.trailEl.offsetHeight / 2
        : this.cy;
      // trail을 살짝 뒤따르게
      const k = 0.85;
  ...
  _onEnter = () => {
    ...
    if (this.hasGsap) {
      ...
      if (this.trailEl)
        window.gsap.to(this.trailEl, { autoAlpha: 1, duration: 0.2 });
    } else {
  ...
  _onLeave = () => {
    if (this.hasGsap) {
      ...
      if (this.trailEl)
        window.gsap.to(this.trailEl, { autoAlpha: 0.4, duration: 0.2 });
    } else {
